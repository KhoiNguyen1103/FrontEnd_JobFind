...
const MenuMessage = ({ userId, onClose }) => {
    ...
    useEffect(() => {
        ...
        const handleMetaUpdate = (data) => {
            ...
            setConversations((prev) => {
                ...
                if (!updatedConversations.some(conv => conv.conversationId === data.conversationId)) {
        ...
        const fetchConversations = async () => {
            try {
                setLoading(true);
                const conversationsResponse = await conversationApi.getUserConversations(user.id);
    ...
    const handleSelectConversation = (conversationId, senderName, senderAvatar, roleId) => {
        const wsService = WebSocketService.getInstance();
                    ...
                    {conversations.map(conv => {
                        ...
                        return (
                            <li
                                ...
                                onClick={() => handleSelectConversation(conv.conversationId, conv.senderName, conv.senderAvatar, conv.roleId)}

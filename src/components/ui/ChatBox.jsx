...
const ChatBox = ({
    ...
    const fetchConversationData = async () => {
      try {
        ...
        const messagesResponse =
          await conversationApi.getMessagesByConversationId(conversationId);
    ...
    try {
      ...
      if (!currentConversationId) {
        if (user.role === "COMPANY") {
          ...
          console.log(
            `✅ Tạo conversation mới: conversationId ${currentConversationId}`
          );
          dispatch(
            setConversationId({
              profileId,
          ...
          throw new Error(
            "Không thể tạo conversation: Vai trò người dùng không hợp lệ"
          );
        }
      }
      ...
      if (!sent) {
        throw new Error(
          `Gửi tin nhắn qua WebSocket tới ${destination} thất bại`
        );
      }
  ...
  let globalIndex = 0;
  const allMessages = Object.entries(groupedMessages).flatMap(
    ([_, msgs]) => msgs
  );
  const messagesFromOthers = allMessages.filter(
    (msg) => msg.senderId !== user.id
  );
  const unreadStartIndexInOthers =
    messagesFromOthers.length - initialUnreadCount;
  const unreadStartMessageId =
    messagesFromOthers[unreadStartIndexInOthers]?.messageId;

  const chatDisplayName = conversationInfo?.senderName || displayName;

  return createPortal(
    <div
      className={`fixed bottom-1 right-4 w-80 bg-white border border-gray-200 rounded-xl shadow-xl z-[1000] transition-all duration-300 ${
      ...
      <div className="flex items-center justify-between p-3 bg-blue-700 text-white rounded-t-xl">
        <div className="flex items-center space-x-2">
          <span
            ...
            onClick={() =>
              navigate(
                `/job-seeker-profile/${conversationInfo?.roleId || profileId}`
              )
            }
          >
            {chatDisplayName}
                ...
                <div key={date}>
                  ...
                  {messages.map((msg) => {
                    const showUnreadSeparator =
                      msg.messageId === unreadStartMessageId;
                    const messageBlock = (
                      <div
                        key={msg.messageId}
                        className={`mb-3 flex ${
                            ...
                            <div className="mt-2">
                              {msg.attachment.fileType?.startsWith("image/") ? (
                                <img
                                  ...
                                  onClick={() =>
                                    setPreviewImage(msg.attachment.filePath)
                                  }
                                />
                              ) : (
                                <a
                                ...
                                >
                                  ...
                                  />
                                  <span className="break-all max-w-[180px]">
                      ...
                      <React.Fragment key={msg.messageId}>
                        {showUnreadSeparator && (
                          <div className="text-center my-4 text-xs text-gray-500 relative">
                            ...
                            <span className="bg-gray-100 px-2 z-10 relative">
                              Tin chưa đọc
                            </span>
                          </div>
                        )}

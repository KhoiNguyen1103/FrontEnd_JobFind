...
const stripePromise = loadStripe(
  "pk_test_51STHp8KPbKIM6kWHp1S7ZQiA8511ElZcevYQobIwMAyjInoDh1QCsUPm04ObdkrD5kce4iphk6hhGoH529rqUrzI00VtKosee8"
);
...
const CheckoutForm = ({
  ...
  useEffect(() => {
    if (existingOrder) {
      ...
      if (existingOrder.status === "PENDING" && existingOrder.intentSecret) {
        ...
        existingOrder.status === "succeeded" ||
    ...
    try {
      ...
      const updatedOrder = {
        ...
        paymentInfo: {
          ...
          expiryYear:
            paymentMethodResult.paymentMethod.card.exp_year.toString(),
      ...
      setPaymentError(
        err.message || "Không thể xác nhận thẻ. Vui lòng thử lại."
      );
    } finally {
      setPaymentLoading(false);
    }
  ...
  const handleChargeOrder = async () => {
    ...
    try {
      ...
      if (
        chargeResponse.status === "succeeded" ||
        chargeResponse.status === "COMPLETED"
      ) {
        setPaymentSuccess(true);
      } else {
      ...
      setPaymentError(
        err.message || "Không thể thực hiện thanh toán. Vui lòng thử lại."
      );
    } finally {
      setPaymentLoading(false);
    }
        ...
        <div className="bg-gray-50 p-6 rounded-lg shadow-md">
          ...
          {orderData || planId ? (
            <div className="space-y-6">
              {(() => {
                const displayPlan =
                  orderData || plans.find((p) => p.id === planId);
                ...
                return (
                  <div className="p-6 bg-white rounded-lg shadow-sm space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg text-gray-800 font-semibold">
                        {displayPlan.durationMonths} tháng
                      </span>
                    </div>
                    {orderData && (
                      <div className="flex justify-between items-center">
            ...
            <div className="text-gray-600">
              Vui lòng chọn gói để hiển thị chi tiết.
            </div>
          )}
        </div>
          ...
          {orderData && orderData.paymentInfo && (
            <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-lg shadow-md mb-6 h-56 w-full max-w-md mx-auto">
              ...
              <div className="absolute bottom-8 right-4 text-lg">
                {orderData.paymentInfo.expiryMonth}/
                {orderData.paymentInfo.expiryYear.slice(-2)}
              </div>
          ...
          <div className="space-y-4">
              ...
              <div className="mt-6 space-y-3 bg-white border border-green-200 p-5 rounded-lg shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-base">
                  ...
                  <div>
                    ...
                    <div className="text-gray-800">
                      {order.paymentInfo.subscriptionId}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {order.status === "COMPLETED" && (
              <div className="flex">
                ...
                <div className="text-gray-600 font-light mt-4 ml-3">
                  {order.createdAt}
                </div>
              </div>
            )}
    ...
    const fetchData = async () => {
      try {
        ...
        if (err.response && err.response.status === 400) {
          try {
            const planResponse =
              await subscriptionPlanApi.listAllSubscriptionPlans();
    ...
    <div className="py-10 bg-gradient-to-br from-green-50 via-white to-blue-50 min-h-screen">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold text-center text-gray-800 mb-10">
          Các Gói Dịch Vụ Dành Cho Nhà Tuyển Dụng
        </h1>
        ...
        {paymentError && (
          <div className="text-center text-red-500 mb-6">{paymentError}</div>
        )}
        ...
        {!loading &&
          !error &&
          orders.length > 0 &&
          orders[0].status === "COMPLETED" && <OrderSummary orders={orders} />}
        ...
        {!loading &&
          !error &&
          ((orders.length === 0 && selectedPlanId) ||

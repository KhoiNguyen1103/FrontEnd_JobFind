...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    if (activeTab === "Dashboard") {
      ...
      adminApi
        .getApplicationTrends(trendFilter, trendMonth)
        .then((res) => {
          setTrendChartData({
            labels: res.labels,
      ...
      adminApi
        .getActiveRegions(regionFilter, regionMonth)
        .then((res) => {
          setRegionChartData({
            labels: res.labels,
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    if (approvalFilter === "pending")
      return job.isApproved === false && (!job.note || job.note.trim() === "");
  ...
  const handleApproveJob = async (jobId) => {
    try {
      ...
      setJobs(
        jobs.map((job) =>
          job.jobId === jobId ? { ...job, isApproved: true } : job
        )
      );
                ...
                <TableBody>
                  {paginate(applications, applicationPage).map((app) => (
                    <TableRow key={app.applicationId}>
                      ...
                      <TableCell>
                        {app.jobSeekerProfile.firstName}{" "}
                        {app.jobSeekerProfile.lastName}
                      </TableCell>
                      ...
                      <TableCell>
                        {(() => {
                          const latestStatus =
                            app.statusDTOList?.[app.statusDTOList.length - 1]
                              ?.status;
                          return (
                            <span
                              className={`px-3 py-1 text-sm font-medium rounded-full ${
              ...
              <div className="flex justify-between items-center mt-4">
                <div>
                  ...
                  <Button
                    ...
                    disabled={
                      applicationPage * itemsPerPage >= applications.length
                    }
                    variant="outlined"
                    className="ml-2"
                  >
          ...
          <Card>
            <CardContent>
              ...
              <div className="mb-4 flex gap-2 mt-2">
                ...
                <Button
                  variant={
                    approvalFilter === "approved" ? "contained" : "outlined"
                  }
                  onClick={() => {
                ...
                <Button
                  variant={
                    approvalFilter === "pending" ? "contained" : "outlined"
                  }
                  onClick={() => {
                ...
                <Button
                  variant={
                    approvalFilter === "rejected" ? "contained" : "outlined"
                  }
                  onClick={() => {
                ...
                <Button
                  variant={
                    approvalFilter === "review" ? "contained" : "outlined"
                  }
                  onClick={() => {
            ...
            <CardContent>
              ...
              <div className="flex justify-between items-center mt-4">
                <div>
                  <Button
                    onClick={() =>
                      setJobSeekerPage((prev) => Math.max(prev - 1, 1))
                    }
